...
SELECT (blood_info).blood_type::text || (blood_info).blood_rh::text AS blood_group,
...
SELECT (donors.blood_info).blood_type::text || (donors.blood_info).blood_rh::text AS blood_group,
...
SELECT COUNT(*) * 100.0 / (SELECT COUNT(*) FROM examinations) AS disqualified_precentage
FROM examinations
...
SELECT AVG(donation_count) AS avg_donations_per_donor
